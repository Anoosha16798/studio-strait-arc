        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8 mb-16">
          ...
          <motion.div variants={staggerItem} transition={motionTransition.default}>
            ...
            <div className="space-y-4">
              <a id="email-link" href={`mailto:${siteData.site.email}`} className="text-white/80 hover:text-white transition-colors text-sm flex items-start gap-3 group">
              ...
              {(siteData.site.phones || []).map((phone, i) => (
                <a key={phone} id={i === 0 ? 'call-btn' : undefined} href={`tel:+91${phone.replace(/\D/g, '')}`} className="text-white/80 hover:text-white transition-colors text-sm flex items-start gap-3 group">
                  <svg className="w-5 h-5 mt-0.5 flex-shrink-0 group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
